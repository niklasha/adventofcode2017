package se.hallqvist.adventofcode2017.day8
...
pc dec 916 if vt <= 426"""
  ...
  def part1(s: String) = {
    ...
    val Pattern = """(\w+) (\w+) (-?\d+) if (\w+) ([!<>=]+) (-?\d+)""".r
    val reg = mutable.Map[String, Int]()
    def apply(op: String, arg1: Int, arg2: Int) = op match {
      case "!=" => arg1 != arg2
      case "<" => arg1 < arg2
      case "<=" => arg1 <= arg2
      case "==" => arg1 == arg2
    ...
    def exec(op: String, r: String, arg: Int) = op match {
      case "inc" => reg(r) = reg.getOrElse(r, 0) + arg
      case "dec" => reg(r) = reg.getOrElse(r, 0) - arg
    }
    val prog = rows.map(_ match { case Pattern(r1, op, arg1, r2, cop, arg2) =>
      if (apply(cop, reg.getOrElse(r2, 0), arg2.toInt)) exec(op, r1, arg1.toInt)
    })
    reg.values.toSeq.sorted.last
  }
...
""") }
  ...
  def part2(s: String) = {
    ...
    val prog = rows.map(_ match { case Pattern(r1, op, arg1, r2, cop, arg2) =>
      if (apply(cop, reg.getOrElse(r2, 0), arg2.toInt)) {
        exec(op, r1, arg1.toInt)
        if (reg(r1) > max) max = reg(r1)
      }
    })
    max
  }
