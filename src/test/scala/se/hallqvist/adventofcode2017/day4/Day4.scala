package se.hallqvist.adventofcode2017.day4
...
"""
  ...
  def part1(s: String) = {
    ...
    def valid(p: String) = {
      ...
      p.split("\\s+").map((w) => counts(w) = counts.getOrElse(w, 0) + 1)
      counts.find(_._2 > 1) == None
    }
    rows.count(valid(_))
  }

  "part1" should "satisfy the examples given" in {
    assertResult(2) { part1("""
aa bb cc dd ee
aa bb cc dd aa
aa bb cc dd aaa
    """) }
  ...
  def part2(s: String) = {
    ...
    def valid(p: String) = {
      val counts = mutable.Map[String, Int]()
      p.split("\\s+").map((w) => { val c = w.split("").sorted.mkString(""); counts(c) = counts.getOrElse(c, 0) + 1 })
      counts.find(_._2 > 1) == None
    }
    rows.count(valid(_))    
  }
