package se.hallqvist.adventofcode2017.day6
...
class Day6 extends FlatSpec {
  val input = "4	10	4	1	8	4	9	14	5	1	14	15	0	15	3	5"

  def part1(s: String) = {
    var mem = s.split("\\s+").map(_.toInt)
    val seen = mutable.Set[Seq[Int]]()
    var cnt = 0
    while (!(seen contains mem)) {
      seen += mem
      val n = mem.sorted.last
      val p = mem.indexOf(n)
      val q = n / mem.length
      val r = n % mem.length
      val x = (if (p + r >= mem.length) (p + r) % mem.length else -1)
      mem = mem.zipWithIndex.map({ case (v, i) => (if (i == p) 0 else v) + q + (if (i > p && i <= p + r || i <= x) 1 else 0 ) })
      cnt += 1
    }
    cnt
  }
  ...
  "part1" should "satisfy the examples given" in {
    assertResult(5) { part1("0 2 7 0") }
  }
  "part1" should "succeed" in { info(part1(input).toString) }

  def part2(s: String) = {
    var mem: Seq[Int] = s.split("\\s+").map(_.toInt)
    ...
    cnt - seen.indexOf(mem)
  }

  "part2" should "satisfy the examples given" in {
    assertResult(4) { part2("0 2 7 0") }
  }
