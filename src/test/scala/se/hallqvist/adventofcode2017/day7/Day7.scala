package se.hallqvist.adventofcode2017.day7
...
"""
  ...
  def part1(s: String) = {
    ...
    val Pattern = """(\w+) \((\d+)\)( -> (.*))?""".r
    val t = mutable.Map[String, Set[String]]()
    rows.map(_ match { case Pattern(n, w, _, s) => t(n) = if (s == null) Set() else s.split(", ").toSet})
    (t.keySet -- t.values.flatten).head
  }

  "part1" should "satisfy the examples given" in {
    assertResult("tknk") { part1("""
pbga (66)
xhth (57)
ebii (61)
havc (66)
...
""") }
  ...
  def part2(s: String) = {
    val rows = s.split("\n").map(_.trim).filter(!_.isEmpty)
    val Pattern = """(\w+) \((\d+)\)( -> (.*))?""".r
    val t = mutable.Map[String, (Int, Set[String])]()
    rows.map(_ match { case Pattern(n, w, _, s) => t(n) = (w.toInt, if (s == null) Set() else s.split(", ").toSet)})
    val root = (t.keySet -- t.values.map(_._2).flatten).head
    def weightSum(s: Set[String]) = s.toSeq.map(weight(_)).sum
    def weight(n: String): Int = t(n) match { case (w, s) => w + weightSum(s) }
    // Return either Some(the fixed weight), or if already balanced: None
    def balance(n: String): Option[Int] = t(n) match { case (w, s) => {
      val perWeight = mutable.Map[Int, Set[String]]()
      s.toSeq.map((c) => { val w = weight(c); perWeight(w) = perWeight.getOrElse(w, Set()) + c })
      if (perWeight.size == 1) None else {
        val toFix = perWeight.find(_._2.size == 1).get._2.head
        Some(balance(toFix) match { case Some(w) => w case None => perWeight.find(_._2.size != 1).get._1 - weightSum(t(toFix)._2) })
      }
    }}
    balance(root).get
  }
